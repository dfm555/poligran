...
class MasterController {
	...
	static function execute($request) {
		...
		$controller = (isset($request['controller']) && $request['controller'] !='') ? ucfirst($request['controller']) . ucfirst(static::$controllerSuffix) : 'index'.ucfirst(static::$controllerSuffix) ;
		$action = isset($request['action']) && $request['action'] !='' ? $request['action'] : static::$defaultMethod;
